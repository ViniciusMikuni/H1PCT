...
parser.add_argument('--nevts', type=float,default=60e6, help='Number of events to load')
...
global_names = ['Q2','e_px','e_py','e_pz','jet_pt','jet_eta','jet_phi']
...
for name,tag in zip(mc_names,mc_tags):
    ...
    global_vars['data'] = np.concatenate([np.expand_dims(data[var][hvd.rank():nevts:hvd.size()],-1) for var in global_names],-1)
