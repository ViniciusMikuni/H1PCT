...
test_data = tf.data.Dataset.from_tensor_slices((test['X'],test['y'])).shard(hvd.size(), hvd.rank()).batch(flags.batch)


inputs,outputs = PCT(train['X'].shape[1],train['X'].shape[2],nheads=10)

callbacks=[
    hvd.callbacks.BroadcastGlobalVariablesCallback(0),
    hvd.callbacks.MetricAverageCallback(),
    hvd.callbacks.LearningRateWarmupCallback(initial_lr=5e-5*hvd.size(), warmup_epochs=3, verbose=0),
...
opt = tf.keras.optimizers.Adam(5e-5*hvd.size())
